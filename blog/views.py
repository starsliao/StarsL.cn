...
class AllTagsView(BaseMixin, ListView):
    template_name = 'blog/all_tag.html'
    context_object_name = 'article_list'

    def get_context_data(self, **kwargs):
        kwargs['tags_list'] = Article.objects.values('tags').distinct().order_by('tags')
        kwargs['PAGE_NUM'] = settings.PAGE_NUM
        return super(AllTagsView, self).get_context_data(**kwargs)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if val == "all":
            ...
            try:
                article_list = Article.objects.filter(tags=val,status=0).order_by(sort)[start:end+1]
#                article_list = Category.objects.get(
#                                   name=val
#                               ).article_set.filter(
#                                   status=0
#                               ).order_by(sort)[start:end+1]
                ...
                logger.error(u'[AllView]此标签不存在:[%s]' % val)
                raise PermissionDenied
...
class NewsView(BaseMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for x in range(start_day, end_day+1):
            ...
            news_list = News.objects.filter(pub_time__gte = datetime.datetime(date.year,date.month,date.day,0,0,0),pub_time__lte = datetime.datetime(date.year,date.month,date.day,23,59,59))
...
def showPPT(request, pptname):
    res = {}
    if os.path.exists(os.getcwd() + '/blog/static/pptx/%s'%pptname):
        slides = os.listdir(os.getcwd() + '/blog/static/pptx/%s/'%pptname)
        res['ppt'] = pptname
#         slides_path = '/static/pptx/%s/'%pptname
        try:
            slides.remove('Thumbs.db')
        except:
            pass
        slides = sorted(slides, cmp=lambda x,y : cmp(int(re.split('\.|_', x)[-2]), int(re.split('\.|_', y)[-2])))
        res['slides'] = slides
        return render_to_response('show_ppt.html', res)
    else:
...
def resume(request):
    res = {}
    print ("go")
    return render_to_response('resume.html', res)
