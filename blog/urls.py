...
from blog.views import (IndexView, ArticleView, AllView, SearchView, AllTagsView,
...
urlpatterns = [
        ...
        url(r'^all_tags/$', AllTagsView.as_view(), name='all-tags-view'),
...
#        url(r'^news/(?P<pk>\w+)$', DetailView.as_view(model=News), name='news-detail-view'),
        url(r'^news/(?P<pk>.*)$', DetailView.as_view(model=News), name='news-detail-view'),
	    ...
	    url(r'^show_ppt/(\w+)', showPPT, name='showPPT'),
	    url(r'resume.html', resume, name='resume'),
]
